...
flags.DEFINE_string('summary_dir', '/tmp/cnn_no_fcl/avg/{}/{}'.format(dataset, dt), 'Summaries directory')
...
def MakeAlexNet(Input, Size, KeepProb):
    ...
    with tf.variable_scope('conv1'):
        ...
        # first pooling
        Pool1 = tf.nn.avg_pool(Norm1, ksize=[1, 3, 3, 1], strides=[
    ...
    with tf.variable_scope('conv2'):
        ...
        # second pooling
        Pool2 = tf.nn.avg_pool(Norm2, ksize=[1, 3, 3, 1], strides=[
                               1, 2, 2, 1], padding='VALID')
    ...
    with tf.variable_scope('conv5'):
        ...
        # fifth pooling
        Pool5 = tf.nn.avg_pool(ReLU5, ksize=[1, 3, 3, 1], strides=[
                               1, 2, 2, 1], padding='VALID')
...
conf.gpu_options.per_process_gpu_memory_fraction = 0.69  # fraction of GPU used

with tf.device('/gpu:90'):
