...
flags.DEFINE_string('summary_dir', '/tmp/alexnet/cnn_no_fcl/avg/{}/{}'.format(dataset, dt), 'Summaries directory')
...
def MakeAlexNet(Input, Size, KeepProb):
	...
	with tf.variable_scope('conv1'):
		...
		# first pooling
		Pool1 = tf.nn.avg_pool(ReLU1 , ksize=[1, 3, 3, 1], strides=[
	...
	with tf.variable_scope('conv2'):
		...
		# response normalization
		# second pooling
		Pool2 = tf.nn.avg_pool(ReLU2 , ksize=[1, 3, 3, 1], strides=[
							   1, 1, 1, 1], padding='VALID')
	...
	with tf.variable_scope('conv5'):
		...
		# fifth pooling
		Pool3 = tf.nn.avg_pool(ReLU5, ksize=[1, 3, 3, 1], strides=[
							   1, 1, 1, 1], padding='VALID')
	...
	with tf.Session(config=conf) as Sess:
		...
		while Step < NumIteration:
			...
			if (Step%EvalFreq)==0:			
				...
				TestAcc = float((1.*TotalAcc)/TestData.shape[0])
			...
			if not Step % SaveFreq:
				...
				np.savetxt('results/cenn_friendly_avgpool{}'.format(run_number), TestAccOut)
